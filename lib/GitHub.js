...
class GitHub extends GitService {
  constructor (options = {}) {
    ...
    return (async () => {
      ...
      const isAppAuth = Boolean(
        config.get('githubAppID') && config.get('githubPrivateKey')
      )
      const isLegacyAuth = Boolean(
        config.get('githubToken') && ['1', '2'].includes(options.version)
      )
      // New: allow server env fallback so callers don't need to pass token each request
      const envToken = process.env.GITHUB_TOKEN || process.env.GH_TOKEN

      let authToken

      if (options.oauthToken) {
        // 1) explicit OAuth token from request
        authToken = options.oauthToken
      } else if (options.token) {
        // 2) explicit personal access token from request (encrypted or plain, depending on your setup)
        authToken = options.token
      } else if (envToken) {
        // 3) server environment fallback (preferred for self-hosted)
        authToken = envToken
      } else if (isLegacyAuth) {
        // 4) legacy static config for v1/v2
        authToken = config.get('githubToken')
      } else if (isAppAuth) {
        // 5) GitHub App installation token
        authToken = await this._authenticate(options.username, options.repository)
      } else {
        throw new Error('Require an `oauthToken` or `token` option (or set GITHUB_TOKEN / githubToken)')
      }
